Page({
    ...
    //     if (blocks.length === 0) {
    //       wx.showToast({ title: '请选择要预约的场地时段' });
    //       return;
    //     }
    ...
    //     // 发送请求
    //     wx.request({
    //       url: 'http://127.0.0.1:8000/course/api/field-book/',
    //       method: 'POST',
    //       header: {
    //         'Authorization': 'Bearer ' + token,   // 关键所在！
    //         'content-type': 'application/json', // 默认值
    //         // 如果你用的是 JWT 或 Token 认证，记得带上 token
    ...
    //         } else {
    //           wx.showToast({ icon: 'none', title: '预约失败，请重试' });
    //           console.error('预约失败:', res.data);
    //         }
    //       },
    //       fail: (err) => {
    //         wx.showToast({ icon: 'none', title: '网络异常，请检查网络' });
